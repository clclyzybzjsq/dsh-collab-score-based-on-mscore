...
if(BUILD_CONFIGURATION STREQUAL "APP-WEB")
    ...
    # Local patch: honor an explicitly requested inspector module off (its QML
    # autogen exceeds the Windows command-line length limit with the deep
    # source path); keep the app-web default of ON otherwise.
    if (MUE_BUILD_INSPECTOR_MODULE)
        set(MUE_BUILD_INSPECTOR_MODULE ON)
    endif()
    # Local patch: with the real inspector module off (stub selected above),
    # still build the stub's QML module so `import MuseScore.Inspector` from
    # the web appshell (NotationFrame.qml) resolves. The stub qml module only
    # ships InspectorForm.qml/TextStylePopup.qml placeholders, so it does not
    # hit the Windows command-line length limit that the full inspector_qml
    # autogen does.
    set(MUE_BUILD_INSPECTOR_QML ON)
