...
// Local patch: contextual interfaces are resolved through the IoC at each use.
template <class I>
std::shared_ptr<I> webResolve(const char* module)
{
    return muse::modularity::ioc()->resolve<I>(module);
}
...
void WebApi::init()
{
    auto globalContext = webResolve<mu::context::IGlobalContext>("context");
    auto onProjectChanged = [this, globalContext]() {
...
void WebApi::load(const void* source, unsigned int len)
{
    ...
    webResolve<muse::actions::IActionsDispatcher>("actions")->dispatch("file-open", actions::ActionData::make_arg1(QUrl::fromLocalFile(tempFilePath.toQString())));
}

void WebApi::addSoundFont(const std::string& uri)
{
    webResolve<muse::audio::ISoundFontController>("audio")->addSoundFont(Uri(uri));
}

void WebApi::startAudioProcessing()
{
    webResolve<muse::audio::IStartAudioController>("audio")->startAudioProcessing(IApplication::RunMode::GuiApp);
}

void WebApi::saveScore()
{
    // Save the current project to the file it was opened from (the temp file
    // written by load()); the saveComplited callback then hands the saved
    // bytes back to JS via onProjectSaved, which the browser half uploads to
    // the score-collab server to persist the session workdir.
    webResolve<muse::actions::IActionsDispatcher>("actions")->dispatch("file-save");
}
