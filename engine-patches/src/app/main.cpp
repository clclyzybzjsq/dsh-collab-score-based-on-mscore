...
#define MUSE_APP_INSTALL_SUFFIX ""
    ...
    // Local patch: Qt 6.7.3 wasm crashes at startup in the widgets platform
    // menu path (QWidgetPlatformMenuItem::setRole -> QItemSelectionModel
    // signal slot resolves to a null function pointer). The web UI is QML and
    // never needs a native menubar, so disable native menus entirely.
#ifdef Q_OS_WASM
...
#else
    LOGI() << "SSL Info: not supported (Qt built without SSL)";
#endif;
