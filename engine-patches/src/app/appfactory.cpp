...
// Modules
// Local patch: the wasm build links src/web/appshell (web AppShellModule), but
// this bare include resolved to the legacy src/appshell header (28-byte layout)
// while the web implementation accesses a 36-byte layout, corrupting the
// adjacent module object (ODR). Include the web header explicitly; appjs below
// already uses the web/ path unconditionally.
#include "web/appshell/appshellmodule.h"
