...
bool QFontProvider::inFont(const Font& f, char32_t ucs4) const
{
    ...
    // Local patch: QChar::fromUcs4 returns char32_t on Qt 6.7; build the
    // single-codepoint string via QString::fromUcs4 instead.
    int glyphIndex = qRawFont.glyphIndexesForString(QString::fromUcs4(&ucs4, 1))[0];
    if (glyphIndex == 0) {
        return false;
    }
