...
// Local patch: upstream declared only ISoundFontRepository, but the WASM branch
// of soundfontrepository.cpp calls promise.onResolve(this, ...), which needs a
// kors::async::Asyncable receiver. Asyncable is a plain lifetime/connection
// base with no interface impact, so the implementation class inherits it too.
