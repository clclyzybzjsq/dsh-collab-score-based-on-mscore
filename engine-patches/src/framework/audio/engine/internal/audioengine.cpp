...
#ifdef Q_OS_WASM
    // Local patch: on wasm (single AudioWorklet thread) the QuickOperation
    // mutex deadlocks. execOperation runs synchronously inside the RPC onmessage
    // handler on the same thread that drives AudioEngine::process(); the
    // scoped_lock in process() (QuickOperation branch) then blocks forever on
    // the mutex this call holds, and m_processing stays true so the initial
    // wait loop above also spins. There is no concurrent audio thread to race
    // with, so run the operation inline without the lock.
    func();
#else
