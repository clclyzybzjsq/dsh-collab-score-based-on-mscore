...
#ifdef Q_OS_WASM
        // Local patch: on wasm (single AudioWorklet thread) waiting for
        // readyToPlayChanged is unsafe — the change is only signalled from
        // processing, which cannot run while this RPC handler blocks the one
        // thread. The synth either is ready already (soundfont loaded before
        // AddTrack) or never will be within this call; treat it as ready and
        // let the failure surface later during playback instead of deadlocking
        // the whole audio engine.
        continue;
#endif
