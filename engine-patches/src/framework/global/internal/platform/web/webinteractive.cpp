...
muse::async::Promise<muse::Ret> WebInteractive::close(const UriQuery& uri)
{
    return m_origin->close(uri);
}

muse::async::Promise<muse::Ret> WebInteractive::close(const Uri& uri)
{
    return m_origin->close(uri);
}

muse::Ret WebInteractive::closeSync(const UriQuery& uri)
{
    return m_origin->closeSync(uri);
}

muse::Ret WebInteractive::closeAllDialogsSync()
{
