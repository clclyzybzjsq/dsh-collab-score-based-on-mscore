    ...
    init(data) {
        ...
        if (this.IS_USED_WORKER) {
            ...
            MuseAudio.mainPort.onmessage = function(event) {
                try {
                    // [worklet-trace] raw msgpack: [1, callId, method, type, ...]
                    const u8 = new Uint8Array(event.data.buffer || event.data);
                    let di = 0;
                    const rd = () => { const b = u8[di]; if (b < 0x80) { di++; return b; } if (b === 0xcc) { di++; return u8[di++]; } if (b === 0xcd) { di++; const v = (u8[di] << 8) | u8[di+1]; di += 2; return v; } if (b === 0xce) { di++; const v = (u8[di] * 0x1000000) + (u8[di+1] << 16) + (u8[di+2] << 8) + u8[di+3]; di += 4; return v; } di++; return -1; };
                    const mId = rd(), cId = rd(), method = rd(), type = rd();
                    const t0 = Date.now();
                    globalThis.debugLog("worklet-rpc method=" + method + " type=" + type + " callId=" + cId + " t0=" + t0);
                    MuseAudio.main_worker_rpcListen(event.data)
                    globalThis.debugLog("worklet-rpc DONE method=" + method + " callId=" + cId + " after=" + (Date.now() - t0) + "ms")
                } catch (error) {
                    globalThis.debugLog("rpcListen ERROR: " + error)
                }
            }
