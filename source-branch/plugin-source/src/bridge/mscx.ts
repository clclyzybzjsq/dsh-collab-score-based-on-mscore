 ...
 */
export function scanXml(source: string, onEvent: (event: XmlEvent) => void): ScanResult {
  const stack: string[] = []
  ...
  while (i < source.length) {
    ...
    while ((attrMatch = attrRe.exec(body)) !== null) {
      ...
      if (key !== undefined) attrs[key] = attrMatch[3] ?? attrMatch[4] ?? ''
    }
  ...
  if (error !== undefined) return { root, wellFormed: false, error }
  if (root === null) return { root: null, wellFormed: false, error: 'document has no root element' }
 ...
 */
export function smokeCounts(mscx: string): SmokeCounts & { wellFormed: boolean; error?: string } {
  ...
  let inNote = false
  let pending: 'tick' | 'pitch' | null = null
  let minPitch = Number.POSITIVE_INFINITY
  let maxPitch = Number.NEGATIVE_INFINITY
  const result = scanXml(mscx, (event) => {
    if (event.type === 'open') {
      switch (event.name) {
        case 'Measure': counts.measures += 1; break
        case 'Note': counts.notes += 1; inNote = true; break
        case 'Chord': counts.chords += 1; break
        case 'Part': counts.parts += 1; break
        case 'Staff': counts.staffs += 1; break
        case 'KeySig': counts.keysigs += 1; break
        case 'TimeSig': counts.timesigs += 1; break
        case 'Tempo': counts.tempos += 1; break
        case 'RehearsalMark': counts.rehearsalMarks += 1; break
        case 'tick': if (inNote) pending = 'tick'; break
        case 'pitch': if (inNote) pending = 'pitch'; break
      }
      return
    }
    if (event.type === 'text' && pending !== null) {
      ...
      if (Number.isFinite(value)) {
        if (pending === 'tick') counts.tickSum += value
        else {
    ...
    if (event.type === 'close') {
      if (event.name === 'Note') inNote = false
      if (event.name === 'tick' || event.name === 'pitch') pending = null
    }
  })
  if (Number.isFinite(minPitch)) counts.pitchRange = [minPitch, maxPitch]
  return {
    ...counts,
 ...
 */
export function projectSummary(mscx: string): ScoreSummary {
  const parts: PartSummary[] = []
  const tempos: TempoRow[] = []
  const timeSignatures: TimeSigRow[] = []
  const keySignatures: KeySigRow[] = []
  ...
  scanXml(mscx, (event) => {
    if (event.type === 'open') {
      switch (event.name) {
        case 'Part': inPart = true; partNameTaken = false; break
        case 'trackName': if (inPart && !partNameTaken) trackNamePending = true; break
        case 'Measure': measureIndex += 1; break
        case 'tick': tickPending = true; break
        case 'Tempo': inTempo = true; break
        case 'tempo': if (inTempo) tempoPending = true; break
        case 'TimeSig': inTimeSig = true; sigN = 0; sigD = 0; break
        case 'sigN': if (inTimeSig) readingSigN = true; break
        case 'sigD': if (inTimeSig) readingSigD = true; break
        case 'KeySig': inKeySig = true; keyAccidentals = 0; break
        case 'accidental': if (inKeySig) keyAccidentals += 1; break
        case 'RehearsalMark': rehearsalMarks += 1; break
      }
      return
    }
    if (event.type === 'text') {
      ...
      if (tickPending) {
        ...
        if (Number.isFinite(parsed)) tickValue = parsed
        return
      }
      if (tempoPending) {
        const parsed = Number.parseFloat(text)
        if (Number.isFinite(parsed)) tempoValue = parsed
        return
      }
      if (readingSigN) {
        const parsed = Number.parseInt(text, 10)
        if (Number.isFinite(parsed)) sigN = parsed
        return
      }
      if (readingSigD) {
        const parsed = Number.parseInt(text, 10)
        if (Number.isFinite(parsed)) sigD = parsed
        return
      }
      return
    }
    if (event.type === 'close') {
      switch (event.name) {
        ...
        case 'trackName':
          ...
          partNameTaken = true
          break
        case 'tick': tickPending = false; break
        case 'Tempo':
          tempos.push({ tick: tickValue, tempo: tempoValue })
          inTempo = false
          break
        case 'tempo': tempoPending = false; break
        case 'TimeSig':
          timeSignatures.push({ measure: measureIndex, sigN: sigN || 4, sigD: sigD || 4 })
          inTimeSig = false
          break
        case 'sigN': readingSigN = false; break
        case 'sigD': readingSigD = false; break
        case 'KeySig':
          keySignatures.push({ measure: measureIndex, accidentals: keyAccidentals })
          inKeySig = false
          break
        case 'accidental': readingSigN = false; break
      }
    }
  })
  return {
...
// `smokeCounts` reports well-formedness from its own structural scan; no
// separate verdict pass is needed.

/** One note row of the `--depth notes` projection. */
export interface NoteRow {
  /** Location locator, isomorphic with MuseScore's `--score-elements` location. */
  loc: { staffIdx: number; measureIdx: number; voiceIdx: number }
  pitch: number
  tpc: number
  duration: string
  dots: number
...
const noteLine = (note: NoteRow): string =>
  `m${note.loc.measureIdx}:s${note.loc.staffIdx}:v${note.loc.voiceIdx} p${note.pitch}@${note.duration}${note.dots > 0 ? `.${note.dots}` : ''}`
 ...
 */
export function projectNotes(mscx: string): NotesProjection {
  const notes: NoteRow[] = []
  let staffIdx = 0
  let measureIdx = 0
  let voiceIdx = 0
  ...
  let currentVoice = -1
  let inNote = false
  let note: { pitch: number; tpc: number; duration: string; dots: number; tick: number } | null = null
  let pending: 'pitch' | 'tpc' | 'duration' | 'dots' | 'tick' | null = null
  ...
  scanXml(mscx, (event) => {
    if (event.type === 'open') {
      switch (event.name) {
        case 'Staff': currentStaff = staffIdx; staffIdx += 1; break
        case 'Measure': currentMeasure = measureIdx; measureIdx += 1; break
        case 'voice':
          currentVoice = voiceIdx
          voiceIdx += 1
          break
        case 'Note': inNote = true; note = { pitch: 0, tpc: 0, duration: '', dots: 0, tick: 0 }; break
        case 'pitch': if (inNote) pending = 'pitch'; break
        case 'tpc': if (inNote) pending = 'tpc'; break
        case 'durationType': pending = 'duration'; break
        case 'dots': if (inNote) pending = 'dots'; break
        case 'tick': if (inNote) pending = 'tick'; break
      }
      return
    }
    if (event.type === 'text') {
      if (pending === null || note === null) return
      const value = Number.parseInt(event.text, 10)
      switch (pending) {
        case 'pitch': if (Number.isFinite(value)) note.pitch = value; break
        case 'tpc': if (Number.isFinite(value)) note.tpc = value; break
        case 'duration':
          lastDuration = event.text
          note.duration = event.text
          break
        case 'dots': if (Number.isFinite(value)) note.dots = value; break
        case 'tick': if (Number.isFinite(value)) note.tick = value; break
      }
      return
    }
    if (event.type === 'close') {
      switch (event.name) {
        case 'Note':
          ...
          note = null
          break
        case 'Staff': currentStaff = -1; break
        case 'Measure': currentMeasure = -1; break
        case 'voice': currentVoice = -1; break
        case 'pitch': case 'tpc': case 'durationType': case 'dots': case 'tick': pending = null; break
      }
    }
  })
...
export function diffLines(a: string[], b: string[]): DiffLine[] {
  ...
  for (let k = 0; k < width; k++) {
    ...
    const right = b[k]
    if (left === right) {
      if (left !== undefined) rows.push({ kind: 'unchanged', text: left })
    } else {
