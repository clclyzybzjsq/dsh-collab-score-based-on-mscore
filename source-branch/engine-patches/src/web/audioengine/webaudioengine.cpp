...
{
    // Local patch: wasm runs the audio engine on a single AudioWorklet thread
    // with no separate main/audio loop, so kors::async queues (used by
    // Promise AsyncByPromise bodies and channel callbacks) are never pumped.
    // Process them here, once per audio callback, so RPC promises resolve.
