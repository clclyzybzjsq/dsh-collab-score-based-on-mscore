...
void AppMenuModel::load()
{
    ...
    // Local patch (dsh): drop the desktop file-menu (save) and the dev
    // diagnostics menu (file-open); the dsh plugin page owns file/save. Keep
    // the editing/input/tool menus the notation editor needs.
    MenuItemList items {
        makeEditMenu(),
        makeViewMenu(),
        makeAddMenu(),
