...
{
    qInfo() << "[appshell] layout this" << (void*)this
            << "&aac" << (void*)&m_applicationActionController
            << "&aua" << (void*)&m_applicationUiActions
            << "&asc" << (void*)&m_appShellConfiguration
            << "sizeofObj" << sizeof(*this)
            << "sizeofSP" << sizeof(std::shared_ptr<int>);
    qInfo() << "[appshell] rawnew-Config @" << (void*)new AppShellConfiguration(iocContext());
    dbgCheckBrailleAt("after-rawnew-Config");
    qInfo() << "[appshell] registerExports: ApplicationActionController";
    m_applicationActionController = std::make_shared<ApplicationActionController>(iocContext());
    dbgCheckBrailleAt("after-ActionController");
    qInfo() << "[appshell] registerExports: ApplicationUiActions";
    m_applicationUiActions = std::make_shared<ApplicationUiActions>(m_applicationActionController, iocContext());
    dbgCheckBrailleAt("after-UiActions");
    qInfo() << "[appshell] registerExports: AppShellConfiguration";
    m_appShellConfiguration = std::make_shared<AppShellConfiguration>(iocContext());
    dbgCheckBrailleAt("after-Configuration");
    qInfo() << "[appshell] registerExports: registerExport AppShellConfiguration";
    ioc()->registerExport<IAppShellConfiguration>(moduleName(), m_appShellConfiguration);
    dbgCheckBrailleAt("after-registerExport");
    qInfo() << "[appshell] registerExports: StartupScenario";
    // Restored: TEMP BISECT disabled this while hunting the heap corruption,
    // but the real culprit was the appfactory ODR layout mismatch (now fixed).
    // GuiApp::applyCommandLineOptions unconditionally calls startupScenario()->
    // setStartupType(), so this registration must stay enabled.
    ioc()->registerExport<IStartupScenario>(moduleName(), new StartupScenario(iocContext()));
    dbgCheckBrailleAt("after-StartupScenario");
    qInfo() << "[appshell] registerExports: done";
}
