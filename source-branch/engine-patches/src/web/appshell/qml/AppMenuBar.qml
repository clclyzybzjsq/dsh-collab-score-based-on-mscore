...
ListView {
    ...
    delegate: FlatButton {
        ...
        // Local patch: the stock web delegate read `model.itemRole`, but
        // AbstractMenuModel::roleNames() exposes the role as "item" (and has
        // no itemRole() accessor), so item was always undefined and opening
        // any menu threw "Cannot read property 'subitems' of undefined" at
        // menuLoader.open(item.item.subitems). Use the same required-property
        // binding as the desktop platform AppMenuBar.
