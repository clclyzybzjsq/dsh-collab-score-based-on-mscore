...
/** Minimal shape of the tool execution context we read the session id from. */
interface ExecLike {
 ...
 */
function resolveWorkdir(exec: ExecLike, config: Config): string {
...
const renderText = (_args: unknown, value: unknown): { type: 'text'; text: string }[] =>
  [{ type: 'text', text: JSON.stringify(value, null, 2) }]

/** One score_sync result. */
interface SyncOutcome {
  ...
  wellFormed: JsonValue
  measures: number
  notes: number
  error?: string
}

/** One score_view result. */
...
async function probeWorkdir(workdir: string, signal: AbortSignal): Promise<SyncOutcome> {
...
function boundedNotes(mscx: string, cap: number): { notes: unknown[]; count: number; truncated: boolean } {
...
function boundedDiff(workdir: string): Promise<{ rows: unknown[]; from: string; to: string; added: number; removed: number; truncated: boolean }> {
...
async function readScoreWithVerdict(workdir: string): Promise<{ exists: boolean; mscx: string | null; wellFormed: boolean; error?: string }> {
  const container = await readContainer(workdir)
  if (container === null) return { exists: false, mscx: null, wellFormed: false, error: '工作目录尚无 score.mscs' }
  const embedded = mscxOfContainer(container)
  if (embedded === null) return { exists: true, mscx: null, wellFormed: false, error: 'score.mscs 内没有 .mscx 文件' }
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        if (base.mscx === null) return outcome
        if (!base.wellFormed) {
          outcome.error = `结构自检失败（视图仍可读取，但写面不可用）：${base.error ?? 'unknown'}`
          return outcome
        }
        if (args.depth === 'summary') {
          outcome.summary = projectSummary(base.mscx) as unknown as JsonValue
        } else if (args.depth === 'notes') {
          const bounded = boundedNotes(base.mscx, ROW_CAP)
          outcome.notes = { rows: bounded.notes, count: bounded.count, truncated: bounded.truncated } as unknown as JsonValue
          if (bounded.truncated) outcome.info = `音符行超过 ${ROW_CAP} 条，仅返回前 ${ROW_CAP} 条`
        } else {
          try {
            outcome.diff = (await boundedDiff(workdir)) as unknown as JsonValue
          } catch (error) {
