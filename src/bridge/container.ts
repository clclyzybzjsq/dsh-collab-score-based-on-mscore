...
export function crc32(buf: Buffer, start = 0, end = buf.length): number {
  let c = 0xffffffff
  for (let i = start; i < end; i++) c = (CRC_TABLE[(c ^ (buf[i] ?? 0)) & 0xff] ?? 0) ^ (c >>> 8)
...
export function readZip(buf: Buffer): ZipEntry[] {
  ...
  for (let i = 0; i < totalEntries; i++) {
    if (buf.readUInt32LE(p) !== 0x02014b50) throw new Error(`ZIP: central dir entry ${i} signature mismatch`)
...
export function writeZip(entries: Array<{ name: string; data: Buffer }>): Buffer {
  const chunks: Buffer[] = []
  const central: Array<{ nameBuf: Buffer; crc: number; method: number; compSize: number; uncompSize: number; offset: number }> = []
  let offset = 0
...
export function filterMscsEntries(entries: ZipEntry[]): ZipEntry[] {
 ...
 */
export function encodeMscs(entries: Array<{ name: string; data: Buffer | string }>): Buffer {
  const parts: Array<Buffer | string> = ['<?xml version="1.0" encoding="UTF-8"?>\n<files>\n']
...
export function qtTrim(buf: Buffer): Buffer {
 ...
 */
export function decodeMscs(text: Buffer | string): Array<{ name: string; data: Buffer }> {
  const s = Buffer.isBuffer(text) ? text.toString('utf8') : text
  const filesStart = s.indexOf('<files>')
  if (filesStart < 0) throw new Error('mscs: no <files> element')
  const re = /<file name="([^"]+)">\n<!\[CDATA\[([\s\S]*?)\]\]>\n<\/file>\n/g
  ...
  if (entries.length === 0) throw new Error('mscs: no <file> entries found')
