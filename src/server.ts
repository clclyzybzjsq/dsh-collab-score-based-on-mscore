...
/** Async route handler the webserver accepts. */
type RouteHandler = (req: IncomingMessage, res: ServerResponse) => void | Promise<void>

/** Structural view of the webServer service: enough of register() for this bundle. */
interface ScoreWebServer {
  register(route: { kind: 'exact' | 'prefix'; path: string; handler: RouteHandler }): () => void
}

/** One score workdir's state, as published by the state endpoint (M1: full turn detail). */
 ...
 */
async function readWorkdirState(dir: string): Promise<WorkdirState> {
 ...
 */
function makeHandler(workdirRoot: string, engineDir: string): RouteHandler {
 ...
 */
async function ensurePresetInstalled(ctx: Context): Promise<void> {
  const deadline = Date.now() + 60_000
  while (true) {
    const presets = ctx.get('agentPresets') as AgentPresets | null | undefined
    if (presets != null) {
...
async function installOnce(ctx: Context, presets: AgentPresets): Promise<void> {
  if ((await presets.list()).some(preset => preset.id === 'score-collab')) return
  try {
  ...
  const composition = readFileSync(
    new URL('../preset/score-collab/agent.cordis.yml', import.meta.url), 'utf8')
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const webServer = (ctx as unknown as Context & { webServer: ScoreWebServer }).webServer
  ctx.effect(
    () => webServer.register({
      kind: 'prefix',
      path: '/score-collab',
